	...
	if (battleelixirbuffs and battleelixirbuffs[1]) then
		for i, battleelixirbuffs in pairs(battleelixirbuffs) do
			...
			if UnitAura("player", spellname) then
				FlaskFramT.t:SetTexture(select(3, GetSpellInfo(battleelixirbuffs)))
				battleelixired = true
				break
			else
		...
		for i, guardianelixirbuffs in pairs(guardianelixirbuffs) do
			...
			if UnitAura("player", spellname) then
				...
				if not battleelixired then
					FlaskFramT.t:SetTexture(select(3, GetSpellInfo(guardianelixirbuffs)))
				end
				break
			else
...
local function OnAuraChange(self, event, arg1, unit)
	...
	if (flaskbuffs and flaskbuffs[1]) then
		FlaskFramT.t:SetTexture(select(3, GetSpellInfo(flaskbuffs[1])))
		for i, flaskbuffs in pairs(flaskbuffs) do
			local spellname = select(1, GetSpellInfo(flaskbuffs))
			if UnitAura("player", spellname) then
				FlaskFramT.t:SetTexture(select(3, GetSpellInfo(flaskbuffs)))
				FlaskFrame:SetAlpha(0.2)
				break
			else
				CheckElixir()
	...
	if (foodbuffs and foodbuffs[1]) then
		FoodFramT.t:SetTexture(select(3, GetSpellInfo(foodbuffs[1])))
		for i, foodbuffs in pairs(foodbuffs) do
			local spellname = select(1, GetSpellInfo(foodbuffs))
			if UnitAura("player", spellname) then
				FoodFrame:SetAlpha(0.2)
				FoodFramT.t:SetTexture(select(3, GetSpellInfo(foodbuffs)))
				break
			else
	...
	for i, Spell3Buff in pairs(Spell3Buff) do
		...
		if UnitAura("player", spellname) then
			...
			Spell3FramT.t:SetTexture(select(3, GetSpellInfo(Spell3Buff)))
			break
		else
	...
	for i, Spell4Buff in pairs(Spell4Buff) do
		...
		if UnitAura("player", spellname) then
			...
			Spell4FramT.t:SetTexture(select(3, GetSpellInfo(Spell4Buff)))
			break
		else
	...
	for i, Spell5Buff in pairs(Spell5Buff) do
		...
		if UnitAura("player", spellname) then
			...
			Spell5FramT.t:SetTexture(select(3, GetSpellInfo(Spell5Buff)))
			break
		else
	...
	for i, Spell6Buff in pairs(Spell6Buff) do
		...
		if UnitAura("player", spellname) then
			...
			Spell6FramT.t:SetTexture(select(3, GetSpellInfo(Spell6Buff)))
			break
		else
...
local bsize = (((T.minimapsize) - (T.Scale(4) * 7)) / 6)

--Create the Main bar
local raidbuff_reminder = CreateFrame("Frame", "RaidBuffReminder", TukuiMinimap)
raidbuff_reminder:CreatePanel("Default", T.minimapsize, bsize + T.Scale(8), "TOPLEFT", TukuiMinimapStatsLeft, "BOTTOMLEFT", 0, T.Scale(-3))
...
T.SetUpAnimGroup(RaidBuffReminder)
...
local function CreateButton(name, relativeTo, firstbutton)
	...
	button.t = button:CreateTexture(namT..".t", "OVERLAY")
